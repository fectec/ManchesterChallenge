...
class DCMotorNode(Node):
    ...
    def __init__(self):
        ...
        self.declare_parameter("sys_sample_time", 0.0)
        
        # System gain K

        self.declare_parameter("sys_gain_K", 0.0)
        ...
        self.motor_input_sub = self.create_subscription(Float32, "setpoint", self.input_callback, 10)
        ...
        for param in params:
            if param.name == "sys_sample_time":
                if param.value < 0.0:
                    ...
                    return SetParametersResult(successful=False, reason="Sample time cannot be negative.")
                else:
                ...
                if param.value < 0.0:
                    self.get_logger().warn("Invalid value.")
                    return SetParametersResult(successful=False, reason="Time constant cannot be negative.")
                else:
                ...
                if param.value < 0.0:
                    self.get_logger().warn("Invalid value.")
                    return SetParametersResult(successful=False, reason="Initial conditions cannot be negative.")
                else:
