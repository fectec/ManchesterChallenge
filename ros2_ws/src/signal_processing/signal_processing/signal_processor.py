...
class SignalProcessorNode(Node):
    ...
    def process_callback(self):
        ...
        # Modify the signal
        processed_signal = self.amplitude_reduction * (
            self.current_signal * math.cos(self.phase_shift) + 
            math.cos(math.asin(self.current_signal)) * math.sin(self.phase_shift)
        ) + self.alpha
